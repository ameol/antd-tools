...
const os = require('os');
const npmRunner = os.platform() === 'win32'? 'npm.cmd': 'npm';
...
function babelify(js, modules) {
  ...
  let stream = js.pipe(babel(babelConfig))
    .pipe(through2.obj(function z(file, encoding, next) {
      ...
      file.path = file.path.replace(/\\/g, '/'); //window下
...
function compile(modules) {
  ...
  const less = gulp.src(['components/**/*.less'])
    .pipe(through2.obj(function (file, encoding, next) {
      let data = fs.readFileSync(path.resolve(process.cwd(), file.path), 'utf-8');
      data = data.replace(/\/\/\s*@import\s*\'\~antd/g,'@import \'~antd');
      file.contents = Buffer.from(data); // 将less中注释的引入antd组件取消注释
      this.push(file.clone());
      file.path = file.path.replace(/\\/g, '/'); //window下
...
function publish(tagString, done) {
  ...
  const publishNpm = process.env.PUBLISH_NPM_CLI || npmRunner;
...
function pub(done) {
  dist((code) => {
    ...
    if (packageJson.scripts['pre-publish']) {
      runCmd(npmRunner, ['run', 'pre-publish'], (code2) => {
        if (code2) {
          done(code2);
          return;
        }
...
// package.json设置先进行compile再进行pub
