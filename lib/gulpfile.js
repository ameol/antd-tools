...
function compile(modules) {
  ...
  const less = gulp.src(['components/**/*.less'])
    .pipe(through2.obj(function (file, encoding, next) {
      ...
      file.path.match(/\/style\/v2-compatible-reset\.less$/) || file.path.match(/\\style\\v2-compatible-reset\.less$/)) {
...
function publish(tagString, done) {
  ...
  const publishNpm = process.env.PUBLISH_NPM_CLI || 'npm.cmd';
...
function pub(done) {
  dist((code) => {
    ...
    if (packageJson.scripts['pre-publish']) {
      runCmd('npm.cmd', ['run', 'pre-publish'], (code2) => {
        if (code2) {
          done(code2);
          return;
        }
